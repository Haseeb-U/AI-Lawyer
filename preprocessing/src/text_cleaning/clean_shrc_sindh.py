"""
Ultra-Minimal Text Cleaning Script for SHRC Sindh
Preserve-first approach - only removes essential noise
- Normalizes minimal whitespace (3+ newlines → 2)
- Preserves ALL legal content, metadata, and legislative history
- Most conservative cleaner of all sources
...
TEXT_DIR = BASE_DIR / "data" / "text" / "shrc-sindh"
CLEANED_DIR = BASE_DIR / "data" / "cleaned" / "shrc-sindh"
METADATA_FILE = BASE_DIR / "data" / "metadata" / "documents_metadata.json"


class SHRCSindhCleaner:
    """Ultra-minimal cleaner for SHRC Sindh legal documents"""
    ...
    def clean_text(self, text, filename):
        """
        Apply ultra-minimal cleaning while preserving legal content
        Preserve-first approach for already clean SHRC Sindh documents
        """
        original_text = text
        
        # Step 1: Normalize whitespace (ONLY 3+ newlines → 2)
        text = self._normalize_whitespace(text)
        
        # Step 2: Remove excessive separators (ONLY 15+ underscores)
        text = self._remove_separators(text)
        
        # Step 3: Final cleanup (trailing spaces only)
        text = text.strip()
    ...
    def _normalize_whitespace(self, text):
        """Normalize excessive whitespace (ONLY 3+ blank lines)"""
        # Replace 3+ newlines with 2 newlines
        text = re.sub(r'\n{3,}', '\n\n', text)
        
        # Remove trailing spaces from lines (but keep structure)
        text = re.sub(r' +\n', '\n', text)
        
        return text
    
    def _remove_separators(self, text):
        """Remove excessive separator lines (ONLY 15+ chars)"""
        # Remove lines with 15+ underscores/dashes (very conservative)
        text = re.sub(r'\n\s*[_\-]{15,}\s*\n', '\n\n', text)
        return text
    ...
    def _validate_cleaning(self, original, cleaned, filename):
        ...
        # Check 1: Content reduction shouldn't exceed 10%
        ...
        # Check 2: Section count should match (±1%)
        ...
        if original_sections > 0:
            ...
            if section_diff > 0.01:  # Only 1% variance allowed
                self.validation_errors.append({
                    'file': filename,
                    'reason': 'Section numbers lost',
                    'original_sections': original_sections,
                    'cleaned_sections': cleaned_sections
                })
                return False
        
        # Check 3: Ensure notification header preserved
        if 'PROVINCIAL ASSEMBLY' in original and 'PROVINCIAL ASSEMBLY' not in cleaned:
            self.validation_errors.append({
                'file': filename,
                'reason': 'Legislative metadata lost'
            })
            return False
    ...
    def process_all_files(self):
        """Process all SHRC Sindh files that need cleaning"""
        print("=" * 80)
        print("SMART TEXT CLEANING - SHRC SINDH (ULTRA-MINIMAL)")
        print("=" * 80)
        ...
        # Get SHRC Sindh documents that need cleaning
        files_to_clean = []
        
        for doc in metadata.get('documents', []):
            if doc.get('source_website') != 'shrc-sindh':
                continue
        ...
        print(f"📊 Total SHRC Sindh documents: {sum(1 for d in metadata['documents'] if d.get('source_website') == 'shrc-sindh')}")

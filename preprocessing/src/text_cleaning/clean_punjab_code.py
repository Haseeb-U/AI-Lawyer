"""
Smart Text Cleaning Script for Punjab Code
...
- More conservative than Pakistan Code cleaner
"""
...
TEXT_DIR = BASE_DIR / "data" / "text" / "punjab-code"
CLEANED_DIR = BASE_DIR / "data" / "cleaned" / "punjab-code"
METADATA_FILE = BASE_DIR / "data" / "metadata" / "documents_metadata.json"


class PunjabCodeCleaner:
    """Smart cleaner for Punjab Code legal documents"""
    ...
    def clean_text(self, text, filename):
        ...
        More conservative approach for Punjab Code
        """
        original_text = text
        
        # Step 1: Normalize excessive whitespace (main issue in Punjab Code)
        text = self._normalize_whitespace(text)
        
        # Step 2: Clean footnote formatting (but preserve info)
        text = self._clean_footnotes(text)
        
        # Step 3: Remove excessive separators
        text = self._remove_separators(text)
        
        # Step 4: Clean table of contents formatting (optional)
    ...
    def _normalize_whitespace(self, text):
        ...
        # Replace 4+ newlines with double newline (main issue in Punjab Code)
        text = re.sub(r'\n{4,}', '\n\n', text)
        ...
        # Keep single space indentation but remove excessive spaces
        text = re.sub(r'\n {2,}', '\n ', text)
    ...
    def _clean_footnotes(self, text):
        """
        Clean footnote formatting while preserving amendment information
        Punjab Code footnotes are usually inline and important
        """
        # Pattern: Footnote markers like "1[Inserted by Act...]" or "2[Substituted by...]"
        # Keep the amendment info but format it better
        
        # Convert bracket footnotes to cleaner format
        text = re.sub(
            r'\n(\d+)\[([^\]]+)\]',
            r'\n[Amendment \1: \2]',
            text
        ...
        # Remove simple numeric footnote markers that are standalone
        text = re.sub(r'\n\d+\s*\n', '\n', text)
        
        # Clean up excessive footnote numbering at start of lines
        # But keep if followed by actual amendment text
        text = re.sub(r'\n(\d+)([A-Z][a-z]+\.?\s+by\s+)', r'\n(\2', text)
    ...
    def _remove_separators(self, text):
        ...
        # Remove horizontal rules made of special characters
        text = re.sub(r'\n\s*[═─━]{5,}\s*\n', '\n\n', text)
    ...
    def _clean_table_of_contents(self, text):
        ...
        More conservative for Punjab Code
        """
        # Find TOC section
        ...
        def clean_toc_content(match):
            ...
            # Remove excessive dots in TOC (.......)
            content = re.sub(r'\.{5,}', ' ', content)
            
            # Normalize spacing in TOC (3+ spaces to 2 spaces)
            content = re.sub(r' {3,}', '  ', content)
            
            # Remove excessive blank lines in TOC
            content = re.sub(r'\n{3,}', '\n\n', content)
    ...
    def _validate_cleaning(self, original, cleaned, filename):
        ...
        # Check if we removed too much content (more than 30% reduction is suspicious)
        original_length = len(original)
        cleaned_length = len(cleaned)
        
        if cleaned_length < original_length * 0.60:  # Stricter: 60% threshold
            self.validation_errors.append({
                'file': filename,
    ...
    def process_all_files(self):
        """Process all Punjab Code files that haven't been cleaned yet"""
        print("=" * 80)
        print("SMART TEXT CLEANING - PUNJAB CODE")
        print("=" * 80)
        ...
        # Find Punjab Code documents that need cleaning
        files_to_clean = []
        
        for doc in metadata.get('documents', []):
            # Only process punjab-code documents
            if doc.get('source_website') != 'punjab-code':
                continue
        ...
        print(f"📊 Total Punjab Code documents: {sum(1 for d in metadata['documents'] if d.get('source_website') == 'punjab-code')}")
